...
def get_disk():
    res = {}
    disk_info = ps.disk_partitions(all=False)
    res['device'] = {}
    for index, core in enumerate(disk_info):
        res['device'][f"disk_{index}"] = (core.device, core.fstype)
    res["load"] = ps.disk_partitions(all=False)
    return res
...
def show(**kwargs):
    ...
    #SSD
    cpu_time_template = "Device name {2} {1:>10},\t\tFile system {0} {2:>10}\n"
    cpu_time_str = ""
    cpu = kwargs["disk"]
    for key, value in cpu["device"].items():
        cpu_time_str += cpu_time_template.format(key, *value)
    print(cpu_time_str)
